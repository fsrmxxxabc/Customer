...
namespace Customer.Until.Chat
{
    internal static class HtmlFromXamlConverter
    {
        ...
        {
            Trace.WriteLine(xamlString);
            //XmlTextReader xamlReader;
            XmlReader xamlReader;
            StringBuilder htmlStringBuilder;
            XmlWriter htmlWriter;

            using (xamlReader = XmlReader.Create(new StringReader(xamlString)))
            {
                htmlStringBuilder = new StringBuilder(100);
                using (htmlWriter = new XmlTextWriter(new StringWriter(htmlStringBuilder)))
                {
            ...
            while (xamlReader.MoveToNextAttribute())
            {
                ...
                switch (xamlReader.Name)
                {
                    ...
                    case "FontFamily":
                        ...
                        break;
                    case "FontStyle":
                        css = "font-style:" + xamlReader.Value.ToLower(new System.Globalization.CultureInfo("zh-cn")) + ";";
                        break;
                    case "FontWeight":
                        css = "font-weight:" + xamlReader.Value.ToLower(new System.Globalization.CultureInfo("zh-cn")) + ";";
                        break;
                    case "FontStretch":
                        break;
                    case "FontSize":
        ...
        private static string ParseXamlThickness(string thickness)
        {
            ...
            for (int i = 0; i < values.Length; i++)
            {
                ...
                {
                    values[i] = FormattableString.Invariant($"{Math.Ceiling(value)}");
                }
                else
                {
                    values[i] = "1";
                ...
                while (ReadNextToken(xamlReader) && xamlReader.NodeType != XmlNodeType.EndElement)
                {
                    switch (xamlReader.NodeType)
                    {
                        case XmlNodeType.Element:
                            if (xamlReader.Name.Contains(".",StringComparison.OrdinalIgnoreCase))
                            {
            ...
            else
            {
                ...
                switch (xamlReader.Name)
                {
                    ...
                    case "Span":
                        ...
                        break;
                    case "Image":
                        //System.Windows.Controls.Image image = (System.Windows.Controls.Image)System.Windows.Markup.XamlReader.Load(xamlReader);
                        //MessageBox.Show(image.DataContext.ToString());
                        htmlElementName = "Image";
                        break;
                    case "InlineUIContainer":
                        htmlElementName = "SPAN";
                        break;
